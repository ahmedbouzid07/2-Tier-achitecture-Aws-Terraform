...
resource "aws_autoscaling_policy" "scale_up" {
  ...
  scaling_adjustment     = "1" # We increase instance by 1
  cooldown               = "300"
  policy_type            = "SimpleScaling"
}


# Scale up alarm , the alarm will trigger the autoscaling group policy (scale/down) based ont he metric CPUUtilization, comparision_operator, threshold
resource "aws_cloudwatch_metric_alarm" "scale_up_alarm" {
  ...
  period              = "120"
  statistic           = "Average"
  threshold           = "70" # means that a new instance will be created one the CPU utilization is higher than 30%
  dimensions = {
...
resource "aws_cloudwatch_metric_alarm" "scale_down_alarm" {
  ...
  period              = "120"
  statistic           = "Average"
  threshold           = "5" # Instance will scale down when CPU utilisation is lower than 5 %
  dimensions = {
    "AutoScalingGroupName" = aws_autoscaling_group.asg_name.name
  }
  actions_enabled = true
